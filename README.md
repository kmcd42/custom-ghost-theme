...
2. Upload `routes.yaml` in **Settings → Labs → Routes** (or drop it into your
   Ghost `content/settings/` folder). This wires up the `/work/`, `/photography/`,
   `/roles/`, and `/writing/` collections and points `/` at the homepage template.
3. Create the internal tags that drive routing:
   - `#work` — projects / case studies (add a category tag too, e.g. `Advertising`)
   - `#photo` — photography collections (see below)
   - `#role` — work-history entries. Each `#role` post gets its own page at
     `/roles/{slug}/`. Enter the details in the post's **settings sidebar**:
     _title_ = the role, **Excerpt** = the employer, and a visible **tag** =
     the period (e.g. `2023 — Now`). Write the responsibilities as a **bulleted
     list** in the body — they render on the role page and the About page links
     to each one. (Ghost has no free-form custom fields; Excerpt and Tags are
     the structured fields that live in the post settings area.)
4. **Photography** is one post per _collection_: create a post tagged `#photo`
   (+ a category tag), set a **feature image** as the cover, and add all the
   photos as a **gallery** in the post body. The Photography index lists the
   covers; opening one shows the full gallery.
6. Flag up to five `#work` posts as **Featured** — those fill the homepage index.
7. Create Pages with the slugs `about`, `now`, and `colophon`. The About page
   uses a dedicated layout (`page-about.hbs`); Now and Colophon use `page.hbs`
   and are written entirely in the Koenig editor.
8. Set the editable UI copy in **Settings → Design** (see below).
...
| Photo collection   | Post       | `#photo` + a category tag — photos as a gallery in the body |
| Work-history role  | Post       | `#role` — its own page at `/roles/{slug}/`, links from About |
| About / Now / Colophon | Page   | —                                       |

`post.hbs` branches on the post's tags: `#work` → case-study layout,
`#role` → role page (employer, period, bulleted responsibilities),
`#photo` → photo-collection gallery, otherwise → centred long-form article.
